    ...
    -- Library element functions
        function library:window(properties)
            local cfg = { 
                ...
                size = properties.size or properties.Size or dim2(0, 500, 0, 430);
                selected_tab;
                items = {};
            ...
            library:create("UICorner", {Parent = toggle_btn; CornerRadius = dim(0, 10)})
            library:create("UIStroke", {
                Parent = toggle_btn;
                Color = rgb(60, 60, 80);
                ApplyStrokeMode = Enum.ApplyStrokeMode.Border;
            ...
            -- Draggable toggle button with tap detection
            local tog_dragging = false
            local tog_start_pos = nil
            local tog_frame_start = nil
            local tog_moved = false

            toggle_btn.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                    tog_dragging = true
                    tog_moved = false
                    tog_start_pos = input.Position
                    tog_frame_start = toggle_btn.Position
                end
            end)
            ...
            toggle_btn.InputEnded:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                    tog_dragging = false
                    if not tog_moved then
                        menu_open = not menu_open
                        items["main"].Visible = menu_open
            ...
            library:connection(uis.InputChanged, function(input)
                if tog_dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) and tog_start_pos and tog_frame_start then
                    local delta = (input.Position - tog_start_pos)
                    if delta.Magnitude > 6 then
                        tog_moved = true
                    end
                    local vp = camera.ViewportSize
                    local new_pos = dim2(
                        0, math.clamp(tog_frame_start.X.Offset + delta.X, 0, vp.X - 44),
                        0, math.clamp(tog_frame_start.Y.Offset + delta.Y, 0, vp.Y - 44)
                    )
                    library:tween(toggle_btn, {Position = new_pos}, Enum.EasingStyle.Linear, 0.05)
                end
            end)
